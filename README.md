...
- **앱 스크린샷 교체**: `npm run screens -- raw <캡처.png> <이름>`(실기기 캡처, 상태바 제거) / `npm run screens -- store <합성본.png> <이름>`(App Store 합성본, 베젤 안쪽 자동 크롭) / `npm run screens -- full <캡처.jpg> <이름>`(상태바 없는 캡처 — Android 앱 캡처 등, 크롭 없이 리사이즈만) → `public/assets/screens/<이름>.jpg`.
...
- **목표 이벤트**: App Store 링크의 `data-cta` 속성으로 탭이 `appstore_tap` 이벤트(파라미터 `placement`: hero/closing/invite/join, Google Play는 `<placement>-play`)로 잡힌다. 웹앱(app.rougether.com) 진입은 `data-web-cta` → 별도 `webapp_open` 이벤트(placement: hero/section) — 목표 지표와 섞지 않는다. 페이지뷰·유입 채널(utm)·아웃바운드 클릭은 향상된 측정이 자동 수집.
